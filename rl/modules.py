...
class ActionMaskingTorchRLModule(ActionMaskingRLModule, PPOTorchRLModule):
    ...
    def _preprocess_batch(
        ...
        batch[Columns.OBS] = batch[Columns.OBS].pop("observations").to(device)
...
class ActionmMaskingHeuristicsModule(ActionMaskingTorchRLModule):
    ...
    def _current_trick_info(
        ...
        if lead_suit is None:
            ...
            lead_suit = (
                trump_suit
                if lead_action.cards[0].rank in TRUMP_RANKS
            ...
            if team_winning:
                ...
                for idx in candidate_cards:
                    ...
                    if best_card is not None and lead_suit is not None:
                        ...
                        ) or (card.rank in TRUMP_RANKS and lead_suit == trump_suit):
                            if _effective_rank(card) > _effective_rank(best_card):
                                can_beat_on_suit = True

                        # 2b.iii/iv: can we overtrump a non-trump best?
                        if not self._is_trump(
                            best_card, trump_suit_idx
                        ) and self._is_trump(card, trump_suit_idx):
    ...
    def forward_train(self, input_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        vf_preds = torch.zeros(batch_size, dtype=torch.float32, device=logits.device)
        return {"action_dist_inputs": logits, "vf_preds": vf_preds}
